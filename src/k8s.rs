...
macro_rules! register_authenticator {
    ($e:ident) => {
        #[no_mangle]
        fn auth() {
            //let auth: $crate::k8s::Authenticator<S>> = $e;
            //let auth: dyn $crate::k8s::Authenticator<S> = $e;
    ...
    O: Serialize
{
 ...
 pub trait Authenticator<S> {
     ...
     fn runner() -> RequestRunner<TokenReview, TokenReview, S> where S: DeserializeOwned {
        RequestRunner::new(Self::authenticate)
     }
 }

pub trait Authorizer<S> {
    fn authorize(tr: SubjectAccessReview, settings: S) -> Result<SubjectAccessReview, Box<dyn Error>>;

     fn runner() -> RequestRunner<SubjectAccessReview, SubjectAccessReview, S> where S: DeserializeOwned {
...
pub trait Admiter<S> {
    fn admit(tr: (), settings: S) -> Result<(), Box<dyn Error>>;
     ...
     fn runner() -> RequestRunner<(), (), S> where S: DeserializeOwned {
